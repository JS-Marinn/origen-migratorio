...
import { consumerRequirements } from '@/data/consumer-requirements';
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/hablamos-espanol/${service.slug}` },
    openGraph: {
      title,
      description: service.short,
      url: `/hablamos-espanol/${service.slug}`,
    },
    other: {
...
export default async function HablamosEspanolDetalle({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const reqs = consumerRequirements[slug] ?? [];
    ...
    '@graph': [
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Hablamos Español', item: `${SITE_URL}/hablamos-espanol` },
          { '@type': 'ListItem', position: 3, name: service.name, item: `${SITE_URL}/hablamos-espanol/${service.slug}` },
        ],
      },
      {
        '@type': 'Service',
    ...
    <>
      ...
      <section className="relative overflow-hidden bg-navy py-20" aria-label={service.name}>
        <div className="container-om relative max-w-3xl">
          <Reveal>
            <Link href="/hablamos-espanol" className="inline-flex items-center gap-2 text-sm font-semibold text-white/70 transition-colors hover:text-gold-light">
      ...
      {service.bullets && (
        <section className="bg-white py-20" aria-label="Lo que incluye">
          <div className="container-om">
            <Reveal className="mx-auto max-w-2xl text-center">
              <p className="eyebrow justify-center flex">Lo que incluye tu expediente</p>
              <h2 className="mt-4 font-serif text-3xl font-bold text-navy sm:text-4xl">
                Todo listo para que avances con confianza
              </h2>
            </Reveal>
      ...
      <section className="bg-mist py-20" aria-label="Evaluación del caso">
        <div className="container-om">
          <Reveal>
            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-navy to-navy-light p-10 text-center sm:p-14">
              ...
              <p className="relative mx-auto mt-4 max-w-2xl text-base leading-relaxed text-white/85">
                Cada historia migratoria es diferente. Una evaluación profesional puede ayudarte a identificar las opciones disponibles para ti y tu familia.
              </p>
              <p className="relative mx-auto mt-3 max-w-2xl font-semibold text-gold-light">
                Agenda una consulta en español con nuestro equipo y recibe una orientación personalizada.
              </p>
      ...
      <section className="bg-white py-16" aria-label="Otros servicios">
        <div className="container-om">
          <Reveal>
          ...
          <div className="mt-8 grid gap-5 sm:grid-cols-3">
            {others.map((s, i) => (
              <Reveal key={s.slug} delay={i * 90}>
                <Link href={`/hablamos-espanol/${s.slug}`} className="card-lift group flex h-full flex-col rounded-2xl bg-white p-6 ring-1 ring-navy/5">
